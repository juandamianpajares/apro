...
cloud_init_gcp() {
    ...
    # Get GCP project ID
    local gcp_project_id=$(gcloud config get-value project 2>/dev/null)

    if [ -z "$gcp_project_id" ]; then
        warn "GCP project no configurado"
        read -p "Ingresa tu GCP Project ID: " -r gcp_project_id
    ...
    # Create terraform directory
    local tf_project_dir="terraform/gcp/$environment"
    mkdir -p "$tf_project_dir"

    # Copy template
    local template_dir="$TF_DIR/gcp/${project_type}-cloudrun"
...
EOF
    ...
    if terraform init; then
        ...
        echo "  3. Aplica cambios: apro deploy $environment"
        echo ""
        success "Infraestructura GCP lista para deploy"
    else
        error "Terraform init falló"
        return 1
    fi
...
cloud_deploy_gcp() {
    local environment="$1"
    local dry_run="$2"
    local tf_dir="terraform/gcp/$environment"

    if [ ! -d "$tf_dir" ]; then
        error "Directorio de Terraform no encontrado: $tf_dir"
        info "Ejecuta primero: apro init --provider=gcp --type=nextjs"
        return 1
    fi
    ...
    # Confirm
    warn "Esto creará/modificará recursos en GCP"
    read -p "¿Continuar con el deployment? (y/N): " -r confirm
    ...
    if terraform apply tfplan; then
        ...
        info "Siguientes pasos:"
        echo ""
        echo "1. Configura Docker para Artifact Registry:"
        echo ""
        terraform output -json deployment_commands | jq -r '.docker_auth'
        echo ""
        echo "2. Build y push de la imagen:"
        echo ""
        terraform output -json deployment_commands | jq -r '.docker_build'
        echo ""
        terraform output -json deployment_commands | jq -r '.docker_push'
        echo ""
        echo "3. Deploy a Cloud Run:"
        echo ""
        terraform output -json deployment_commands | jq -r '.deploy'
        echo ""
        echo "4. Verificar el deployment:"
        echo ""
        echo "   apro status"
        echo "   apro logs --follow"
        echo ""
...
cloud_destroy_gcp() {
    ...
    info "Destruyendo infraestructura GCP..."
    echo ""
...
cloud_status_gcp() {
    ...
    echo -e "${CYAN}${BOLD}═══════════════════════════════════════════════════${NC}"
    echo -e "${CYAN}${BOLD}GCP Infrastructure Status${NC}"
    echo -e "${CYAN}${BOLD}═══════════════════════════════════════════════════${NC}"
    echo ""
    ...
    # Cloud Run Services
    echo -e "${CYAN}${BOLD}Cloud Run Services:${NC}"
    local services=$(gcloud run services list --platform managed --format="value(metadata.name,status.url,status.conditions.status)" 2>/dev/null)

    if [ -n "$services" ]; then
        while IFS=$'\t' read -r name url status; do
            local status_icon="${GREEN}✓${NC}"
            [ "$status" != "True" ] && status_icon="${YELLOW}⚠${NC}"

            echo -e "  $status_icon $name"
            echo -e "    URL: $url"
            echo -e "    Status: $([ "$status" = "True" ] && echo "Ready" || echo "Not Ready")"
        done <<< "$services"
    else
    ...
    # Cloud SQL Instances
    echo -e "${CYAN}${BOLD}Cloud SQL Instances:${NC}"
    local sql_instances=$(gcloud sql instances list --format="value(name,state,databaseVersion,settings.tier)" 2>/dev/null)

    if [ -n "$sql_instances" ]; then
        while IFS=$'\t' read -r name state version tier; do
            local status_icon="${GREEN}✓${NC}"
            [ "$state" != "RUNNABLE" ] && status_icon="${YELLOW}⚠${NC}"

            echo -e "  $status_icon $name"
            echo -e "    Version: $version | Tier: $tier"
            echo -e "    State: $state"
        done <<< "$sql_instances"
    else
        echo -e "  ${YELLOW}No databases found${NC}"
    ...
    echo -e "${CYAN}${BOLD}Artifact Registry:${NC}"
    local repos=$(gcloud artifacts repositories list --format="value(name,format)" 2>/dev/null)

    if [ -n "$repos" ]; then
        while IFS=$'\t' read -r name format; do
            echo -e "  ${GREEN}✓${NC} $name ($format)"
        done <<< "$repos"
    else
        echo -e "  ${YELLOW}No repositories found${NC}"
    fi
...
cloud_logs_gcp() {
    ...
    info "Obteniendo logs de Cloud Run..."
    echo ""

    # List services
    local services=$(gcloud run services list --platform managed --format="value(metadata.name)" 2>/dev/null)

    if [ -z "$services" ]; then
        warn "No se encontraron servicios de Cloud Run"
        return 1
    fi

    # If service not specified, show available services
    if [ -z "$service" ]; then
        info "Servicios disponibles:"
        echo ""
        echo "$services" | nl
        echo ""
        info "Uso: apro logs [service-name] [--follow] [--tail=100]"
        return 0
    fi

    # Find matching service
    local service_name=$(echo "$services" | grep -i "$service" | head -1)

    if [ -z "$service_name" ]; then
        # Use first service if no match
        service_name=$(echo "$services" | head -1)
        warn "Service '$service' no encontrado, usando: $service_name"
    else
        log "Mostrando logs de: $service_name"
    fi
    echo ""

    # Stream logs
    if [ "$follow" = true ]; then
        gcloud run services logs read "$service_name" --platform managed --follow --format "value(textPayload,jsonPayload.message)"
    else
        gcloud run services logs read "$service_name" --platform managed --limit "$tail" --format "value(textPayload,jsonPayload.message)"
    fi
}
...
cloud_scale_gcp() {
    ...
    # Find Cloud Run service
    local service_name=$(gcloud run services list --platform managed --format="value(metadata.name)" 2>/dev/null | grep -i "$environment" | head -1)

    if [ -z "$service_name" ]; then
        error "No se encontró servicio Cloud Run para ambiente: $environment"
        return 1
    fi
    ...
    if [ -n "$min" ]; then
        update_args+=(--min-instances "$min")
        info "Actualizando min instances a: $min"
    fi

    if [ -n "$max" ]; then
        update_args+=(--max-instances "$max")
        info "Actualizando max instances a: $max"
    fi

    if [ ${#update_args[@]} -eq 0 ]; then
        warn "No se especificaron cambios de scaling"
        return 1
    fi
    ...
    if gcloud run services update "$service_name" \
        --platform managed \
        "${update_args[@]}" &>/dev/null; then

        log "Scaling actualizado"
        echo ""
        success "Scaling completado exitosamente"
        echo ""
        ...
        gcloud run services describe "$service_name" --platform managed \
            --format="value(spec.template.spec.containerConcurrency,spec.template.metadata.annotations.autoscaling\.knative\.dev/minScale,spec.template.metadata.annotations.autoscaling\.knative\.dev/maxScale)" 2>/dev/null | \
            awk '{print "  Min: "$2"\n  Max: "$3"\n  Concurrency: "$1}'
    else
        error "Failed to update scaling"
        return 1
    fi
}
...
cloud_rollback_gcp() {
    ...
    # List revisions
    info "Revisiones disponibles:"
    echo ""

    local revisions=$(gcloud run revisions list --service="$service_name" --platform managed --format="value(metadata.name)" --limit=10 2>/dev/null)

    echo "$revisions" | nl | head -5
    echo ""
    ...
    if [ "$version" = "previous" ]; then
        target_revision=$(echo "$revisions" | sed -n '2p')  # Second most recent
    else
        target_revision=$(echo "$revisions" | grep "$version" | head -1)
    fi

    if [ -z "$target_revision" ]; then
        error "No se encontró revisión para versión: $version"
        return 1
    fi

    info "Haciendo rollback a: $target_revision"
    echo ""

    warn "Esto redirigirá el 100% del tráfico a una revisión anterior"
    read -p "¿Continuar con el rollback? (y/N): " -r confirm
    ...
    # Update traffic
    info "Actualizando tráfico..."

    if gcloud run services update-traffic "$service_name" \
        --platform managed \
        --to-revisions="$target_revision=100" &>/dev/null; then

        log "Tráfico actualizado"
        echo ""
